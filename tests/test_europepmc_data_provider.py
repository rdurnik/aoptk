...
def test_generate_abstract_for_given_query():
    actual = GetEuropePMCPublicationData(str).get_europepmc_json('TITLE_ABS:(liver cancer AND hepg2 AND thioacetamide) AND (FIRST_PDATE:[2018 TO 2019])')
...
def test_is_europepmc_id(id, expected):
    actual = GetEuropePMCPublicationData(str).is_europepmc_id(id)
    assert actual == expected
