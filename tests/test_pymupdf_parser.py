...
# PMC12416454 - test_pdf_abstractcalledabstract.pdf
# PMC12181427 - test_pdf_abstractnotcalledabstractnointroduction.pdf
# Self-made, not on EuropePMC - test_pdf_no_abstract_introduction_keywords_specification.pdf
# PMC12231352 - test_pdf_abstractnotcalledabstract.pdf
...
def text_extract_abstract_fixture(request):
    actual = PymupdfParser(EuropePMC(request.param['id'])).get_publications()[0].abstract
    assert actual == request.param['expected_abstract']
...
def test_extract_abstract_self_made_pdf(path, expected):
...
# Continue with fixtures also for full-text, combine with the previous
@pytest.mark.parametrize(
    ("path", "where", "expected"),
    [
        ("tests/test_pdfs/test_pdf_abstractcalledabstract.pdf", slice(13, 39), "Natural chiral hydrophobic"),
        ("tests/test_pdfs/test_pdf_abstractnotcalledabstract.pdf", slice(1444, 1484), "Metabolic-associated fatty liver disease"),
        ("tests/test_pdfs/test_pdf_abstractnotcalledabstractnointroduction.pdf", slice(108, 127), "Surgical tools used"),
