...
def main(email, use_tg_gates, use_tox21, user_defined_database, query, literature_database, use_mesh_terms):
    ...
    result_df.to_excel(f'src/aoptk/application/{literature_database}_testing_purposes_use_mesh_terms_{use_mesh_terms}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx', index=False)

    output_df = result_df[result_df['relevant_chemicals'].apply(len) > 0]
    output_df.to_excel(f'src/aoptk/application/{literature_database}_identified_chemicals_use_mesh_terms_{use_mesh_terms}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx', index=False)


    exploded_df = output_df.explode('relevant_chemicals')
    grouped_df = exploded_df.groupby('relevant_chemicals').agg(
    ...
    grouped_df.to_excel(f'src/aoptk/application/{literature_database}_grouped_chemicals_use_mesh_terms_{use_mesh_terms}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx', index=False)
